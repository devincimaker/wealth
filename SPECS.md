...
3. **One round trip to the Wealth server** (`server/` in this repo, deployed like thrive): the app posts the audio (or a retry's saved transcript) **plus the ledger context** — active subscriptions, the last ~30 expense notes (newest first), categories, default currency, today. The context travels because the ledger lives on-device: the model resolves spoken references against real records it is shown. The server transcribes with OpenAI **`whisper-1`** (excellent Spanish, auto-detects language — do NOT pin `language: es`, input mixes English), then parses with **Claude Opus 5 via the Agent SDK on subscription auth** (`CLAUDE_CODE_OAUTH_TOKEN`; zero marginal token cost) and returns validated items. The app authenticates with a bearer token; model keys never ship in the app.
4. **Text → structured items**: the server prompts for JSON and validates/normalizes every field before answering, so the app only ever sees clean items. One dictation yields **any number of items** (a 30-second audio naming many spends creates one record each), and each item is an **expense or a subscription** — subscription only on clear recurring language ("subscription", "por mes"), expense when in doubt. Per item: `kind`, `action` (**create/update/delete**), `amount`, `currency` (from "pesos"/"dollars", default last-used), `merchant/note` (the service name for subscriptions; for update/delete it names the existing record), `categoryName?` (matched against my category list, never invented), `date` (relative phrases resolved), `cadence?` (monthly/yearly, subscriptions only). Everything is **saved directly**; a spoken subscription starts on its parsed date and immediately posts the cycle just paid via the normal poster. Updates change the amount of the referenced record ("el café eran 12 mil", "Netflix subió a 18.000"); deletes remove an expense, and **end** a subscription (posting stops, history stays). References resolve by name, case- and accent-insensitive, newest match first, a spoken amount disambiguating; an unresolved reference fails the whole capture without applying any of it. Every capture is undoable: creations are removed, updates reverted, deleted expenses recreated, ended subscriptions revived. One vendor for the whole voice path: same key as Whisper, one bill, one status page.
...
- **Voice quick add**: `AVAudioRecorder` + one `URLSession` call to the Wealth server (`server/`: Whisper to transcribe, Claude Opus 5 on subscription auth to parse). No Apple Speech framework, no Foundation Models — both are poor in Spanish, which is half my dictation. The app carries only the server URL and a bearer token in a local untracked config; model keys live in `server/.env` on the host.
